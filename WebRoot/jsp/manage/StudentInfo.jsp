	...
	<table align="center" cellpadding="5" cellspacing="0">
		...
		<tbody>
			...
			<%
				...
				if(Integer.parseInt(session.getAttribute("managestate").toString())==2){
					int t_id=Integer.parseInt(session.getAttribute("teacherid").toString());
					sqlStr = "select * from stu_re_brief_info_view where LENGTH(s_id_card)>14 and c_name in (select class.c_name from class,t_c_connection where class.c_id=t_c_connection.tc_cla_id and t_c_connection.tc_tea_id="+t_id+") order by st_name desc";
				}else{
					sqlStr = "select * from stu_re_brief_info_view where LENGTH(s_id_card)>14 order by st_name desc";
				}
				String strPage = null;
	...
	<div style="text-align: center;">
		<form action="" method="get" target="_self">
			...
			<%=pageSize%>条/页&nbsp; 第<font color=red><%=showPage%></font>页/共<font
				...
				href="${pageContext.request.contextPath}/jsp/manage/StudentInfo.jsp?showPage=1"
				target="_self">[首 页]</a>&nbsp;
			<%
				//判断"上一页"链接是否要显示
				if (showPage > 1) {
			%>
			<a
				href="${pageContext.request.contextPath}/jsp/manage/StudentInfo.jsp?showPage
=<%=showPage-1%>"
				target="_self">[上一页]</a>&nbsp;
			<%
				} else {
